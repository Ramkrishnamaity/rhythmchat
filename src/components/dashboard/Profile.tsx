...
const Profile: React.FC = () => {
    ...
    try {
      ...
      if (profileData.firstName !== profile?.firstName || profileData.lastName !== profile?.lastName || profileData.about !== profile?.about || image) {
        ...
        if (image) {
          ...
          const response: CommonResponseType<FileUploadResponce> = await postRequest(endpoints.fileUpload, formData, { headers: { authorization: token } }, true)
          if (response.status) {
            setProfileData((prev) => {
        ...
        const response = await putRequest(endpoints.profile, data, { headers: { authorization: token } })
        if (response.status) {
          //update store
  ...
  return (
    <div className='bg-black text-sm space-y-8 sm:w-[calc(70%-6px)] md:w-[calc(70%-10px)] md:px-5 px-3 py-5 w-full h-full rounded-xl'>
      <div className='relative md:w-[25%] w-[35%] mx-auto rounded-full cursor-pointer'>
        {
          !image ? (<img src={profile?.image} className='w-full object-cover rounded-full' />) : (<img src={URL.createObjectURL(image)} className='w-full object-cover rounded-full' />)
        }
      ...
      <div className='relative w-full'>
        <input type='text' readOnly={true} value={profile?.email} className='cursor-not-allowed px-2 py-1 text-xs tracking-widest outline-none border border-[#afb2bf] rounded-sm w-full md:h-[35px] h-[30px] bg-black' />
        <p className='text-[10px] text-blue absolute md:bottom-[24px] bottom-[19px] px-1 bg-black left-2'>Email</p>
      </div>
      <div className='flex md:flex-row flex-col items-center justify-between md:gap-2 gap-8'>
        <div className='relative md:w-[48%] w-full'>
          <input type='text' value={profileData?.firstName} name='firstName' onChange={changeHandler} disabled={disable}
            className='px-2 py-1 text-xs tracking-widest outline-none border border-[#afb2bf] rounded-sm w-full md:h-[35px] h-[30px] bg-black' />
          <p className='text-[10px] text-blue absolute md:bottom-[24px] bottom-[19px] px-1 bg-black left-2'>First Name</p>
        </div>
        <div className='relative md:w-[48%] w-full'>
          <input type='text' value={profileData?.lastName} name='lastName' onChange={changeHandler} disabled={disable}
            className='px-2 py-1 text-xs tracking-widest outline-none border border-[#afb2bf] rounded-sm w-full md:h-[35px] h-[30px] bg-black' />
          <p className='text-[10px] text-blue absolute md:bottom-[24px] bottom-[19px] px-1 bg-black left-2'>Last Name</p>
        </div>
      </div>
      <div className='relative w-full'>
        <input type='text' value={profileData?.about} name='about' onChange={changeHandler} disabled={disable}
          className='px-2 py-1 text-xs tracking-widest outline-none border border-[#afb2bf] rounded-sm w-full md:h-[35px] h-[30px] bg-black' />
        <p className='text-[10px] text-blue absolute md:bottom-[24px] bottom-[19px] px-1 bg-black left-2'>About</p>
      </div>
      <p className='text-xs'>
      ...
      <button onClick={updateProfile} disabled={disable || !isChange}
        className={`${isChange ? 'cursor-pointer' : 'cursor-not-allowed'} bg-blue mx-auto rounded-md sm:px-2 px-1 py-1 sm:py-[6px] flex items-center justify-between gap-2`}>
        {
